...
We first study the events assuming that all measured observables $\vecy$ are equal to their true values.
...
The decision to use LO ME is motivated by the need to reduce the computing-time requirements of the MEM,
the discussion of which concludes the section on the performance of the MEM.

The separation between the $\dihiggs$ signal and the $\ttbar$ background is studied using samples of signal and background events produced by Monte Carlo (MC) simulation.
About fifty thousand $\dihiggs$ signal and about one million $\ttbar$ background events are used for this study.
Separate samples of events have been simulated at LO and at NLO accuracy in pQCD.
...
We quantify the effect of using LO ME by comparing the separation between the $\dihiggs$ signal and the $\ttbar$ background
for signal and background events simulated at LO and at NLO accuracy in pQCD.
The results are shown in Fig.~\ref{fig:memLR_LO_vs_NLO}.
...
%Using ME of orders higher than LO in the MEM seems prohibitive to us.
%When using ME generated at NLO accuracy in pQCD with the program MadGraph\_aMCatNLO 2.3.3 for the $\dihiggs$ signal and the $\ttbar$ background,
%the computing time increases by about a factor $XXX$, from one to about $XXX$ minutes per event,
