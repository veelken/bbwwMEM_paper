...
\end{equation}
\end{linenowrapper}
with $\sigma_{\rho} = XX.X$~\GeV for when computing the PDs $w_{0}(\vecy)$ and $w_{1}(\vecy)$ for $\dihiggs$ signal as well as for $\ttbar$ background events.
...
Our conclusion from these numbers is that by exploiting the full difference in event kinematics between the $\dihiggs$ signal and the $\ttbar$ background with the MEM
one can improve the separation of the $\dihiggs$ signal from the $\ttbar$ background significantly compared to a simple one-dimensional cuts on single observables.
