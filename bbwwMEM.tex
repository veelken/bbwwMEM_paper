...
and study the separation between the $\dihiggs$ signal and the large irreducible $\ttbar$ background arising from $\ttbar$ production.
Focusing on events containing two electrons or muons in the final state,
...
Further studies employing detailed simulations of the ATLAS and CMS detectors are necessary, however, 
to access the possible degradation in separation power that may be caused by events
in which one of the two $\Pbottom$-jets that are produced in the decays of the $\PHiggs$ boson or top quark pair
fails to get reconstructed and a light quark or gluon jet, produced by either initial or final state radiation, from the underlying event, 
...
Code:
\begin{itemize}
\item \textcolor{red}{Internal comments}
\item \textcolor{blue}{New text (Xanda)}
\item \st{remove}
\end{itemize}
